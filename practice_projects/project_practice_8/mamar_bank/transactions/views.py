...
def transfer(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            elif not UserBankAccount.objects.filter(account_no=account_number).exists():
                messages.warning(request,"Invalid Account Number")                
            else:
                account = request.user.account
                account.balance -= amount
                account.save()
                account2 = UserBankAccount.objects.get(account_no=account_number)
                account2.balance += amount
                account2.save()
                send_transaction_email(account.user, amount, "Transfer Message", "transactions/transfer_sender_email.html")
                send_transaction_email(account2.user, amount, "Transfer Messade", "transactions/transfer_reciever_email.html")
